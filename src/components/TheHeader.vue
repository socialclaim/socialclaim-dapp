  ...
  <header class=" w-full px-2 md:py-1 wave-bottom-black bg-secondary bg-secondarymedium  sm:px-4">
    <div class="container flex items-center justify-between mx-auto max-w-screen-lg">
      ...
      <ul class="relative space-x-2 flex rtl:space-x-reverse h-full">
        ...
        <li class="hidden lg:block">
          <a href="https://twitter.com/Verfied_chain" target="_blank"><img src="/images/logos/twitter.svg"  class="block object-contain h-10" /></a>
        </li>
        <div v-if="isActivated" class="flex ">
          <div class="sm:hidden  px-3 rounded-2xl inline-block bg-gray-100">
            {{ shortenAddress(address) }}
          </div>

          <div class="hidden sm:flex px-2 flex items-center rounded-2xl border border-solid">
            <div class="px-1 text-white">{{ displayEther(balance) }} TBNB</div>
            <div class=" px-3 rounded-lg inline-block bg-gray-100">
              {{ shortenAddress(address) }}
            </div>
          </div>
        </div>

        <button v-else @click="open" type="button" class=" text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700 mr-2 mb-2">
