  ...
  <template>
    <section class="wave-bottom-black bg-secondary" id="report">
      <div class="max-w-screen-md mx-auto">
        <div class="max-w-screen-lg mx-auto pt-10">

          <h2 class="text-3xl text-white mb-6  text-center">Verifications report per address</h2>
        </div>
        ...
        <div v-else>
          <div class="flex items-center">
            ...
            <div class="relative w-full">
              <div class="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                <svg v-if="loading" class="inline w-5 h-5 mr-2 text-gray-100 animate-spin dark:text-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <svg v-else class="w-5 h-5 text-gray-500 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd"></path></svg>
              </div>
              <input v-model="searchAddress" type="text" id="simple-search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="a BSC testnet address" required>
            </div>
          ...
          <div class="text-sm text-white">
            Load <a @click="loadAddress()" class="underline cursor-pointer">your address</a> or an <a @click="loadSample()" class="underline cursor-pointer">example address</a> with verifications
          </div>
          <div class="mt-6 mx-auto  pb-20 ">
            <div class=" shadow-md " v-if="searchResults.length > 0">
              <div  class=" w-full bg-white rounded-lg  shadow-md  dark:bg-gray-800 dark:border-gray-700">
                <div class="p-4 mb-2 text-sm text-blue-700 bg-blue-100 rounded-lg dark:bg-blue-200 dark:text-blue-800" role="alert">
                  <span class="font-medium">ℹ️ For now it's your responsability to manually check that the verified <b>HTML selector</b> matched a field <b>only editable by the owner of the account</b> at the given <b>date</b>. This won't be required anymore when the DAO is live.</span>
                </div>
              </div>
  ...
  export default {
    ...
    mounted() {
      this.verifiedContract.on("VerificationForAddress", (addr, timestamp, url, selector) => {
        this.loading = false
        if (this.currentSearchAddress === addr)
      ...
      getVerificationsForAddress() {
        ...
        try {
          ...
          this.verifiedContract.connect(signer.value).getVerificationsForAddress(this.searchAddress).then(function(){
            _self.currentSearchAddress = _self.searchAddress
            setTimeout(function () {
              _self.loading = false
            }, 10000)
          }, function(){
