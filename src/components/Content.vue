...
<template>
  ...
  <div class="flex flex-wrap" v-if="location">
    <div class="w-full lg:w-8/12 bg-primary-800 py-6 px-6 rounded-3xl">
      ...
      <div v-if="location.state !== 'active'" class="p-2">
        <div class="inline-block py-2 min-w-full grid grid-rows-2 md:grid-cols-2 gap-3">
          <div class="shadow-md rounded-lg border border-primary-700 shadow-md  bg-primary-800 hover:bg-primary-700">
            <a v-on:click="openRequestModal()" class=" cursor-pointer flex block p-2 w-full rounded-lg  align-middle ">
              ...
              <div class="full">
                ...
                <div v-if="location.create_requests" class=" italic text-sm font-normal text-primary-400">This city has been requested <br />{{location.create_requests.length}} time(s) before</div>
              </div>
            </a>
          </div>
          <div class="shadow-md rounded-lg border border-primary-700 pl-5">
            ...
            <a href="https://calendly.com/nomadgroups/become-an-embassador"  type="button" class="my-5 mr-2 focus:ring-4 focus:ring-primary-300 rounded-lg border text-sm font-medium px-5 py-2.5 focus:z-10 text-white hover:bg-primary-600 inline-flex items-center">
...
<!--        <div v-if="location.state === 'active'" class="rounded-3xl px-4 py-4 mx-4 mb-4 bg-primary-900 bg-opacity-80">-->
...
<!--        </div>-->
        <div class="rounded-3xl px-4 py-4 mx-4 mb-4 bg-primary-900 bg-opacity-80" v-if="location && (location.links && location.links.length > 0 || location.geonames_feature && location.geonames_feature.links && location.geonames_feature.links.length > 0)" >
          ...
          <ul>
            ...
            <li v-for="m in location.geonames_feature.links" :key="m.id">
              <div class=" mt-4">
                <a :href="m.alternate_name" target="_blank" class="bg-white text-primary-900 rounded-md  py-2 px-4 ">
