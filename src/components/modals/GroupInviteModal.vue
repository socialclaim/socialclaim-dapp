    ...
    <div class="relative px-4 w-full max-w-2xl h-full md:h-auto">
      <div class="relative rounded-lg shadow bg-primary-700">
        ...
        <div class="p-6 rounded-t ">
          <div class="rounded-md bg-primary-800 text-center px-6 py-1 border-2 border-white overflow-x-scroll">
            <a target="_blank" :href="inviteUrl" class="text-xl font-extrabold  leading-relaxed text-white">
              {{inviteUrl}}
            </a>
          </div>
          <div class="text-right text-xs">
            <a v-on:click="openRequestModal()" class=" cursor-pointer text-white underline hover:text-primary-200 ">Report a broken link</a>
          </div>
        </div>
        <div class="rounded-md border-b border-primary-600 text-center px-6 py-1 ">
        ...
        <div class="flex items-center p-6 space-x-2 rounded-b border-t border-primary-600">
          <button data-modal-toggle="default-modal" type="button" class="focus:ring-4 focus:ring-primary-300 rounded-lg border  text-sm font-medium px-5 py-2.5  focus:z-10 text-white hover:bg-primary-600" v-on:click="closeModal()">OK</button>
        </div>
      </div>
    </div>
  </div>
