<template>
  <div  id="group-invite-modal"  style="z-index: 99999"  class="max-w-xl  mx-auto overflow-y-auto overflow-x-hidden fixed right-0 left-0 top-4 z-20 justify-center items-center h-modal md:h-full md:inset-0">
    <div class="relative px-4 w-full max-w-2xl h-full md:h-auto">
      <div class="relative rounded-lg shadow bg-gray-700">
        ...
        <div class="px-6 pb-6 rounded-t border-b border-gray-600">
          ...
          <label for="email-adress-icon" class="block mb-2 pt-6 text-sm font-medium text-gray-400">Invite link</label>
          <div class="relative mt-1">
            <div class="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
              <svg class="w-5 h-5 text-gray-500 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z"></path><path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z"></path></svg>
            </div>
            <input type="text" v-model="group.invite_url" id="email-adress-icon" class="border border-gray-300 text-sm rounded-lg block w-full pl-10 p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500" placeholder="https://...">
          </div>
          <label class="block mb-2 pt-6 text-sm font-medium text-gray-400">Type :</label>
          <select  v-model="group.type" id="countries" class="border text-sm rounded-lg  block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500">
        ...
        <div class="p-6 space-y-6">
          <div class="text-base leading-relaxed text-gray-400">
            Groups guidelines for admins :
            <ul>
              <li>✅ Only post if you are admin of the group</li>
              <li>✅ Make sure the group is not full or nearly full</li>
              <li>✅ Be prepared to moderate the new members</li>
            </ul>
          </div>
        </div>
        <div class="flex items-center p-6 space-x-2 rounded-b border-t border-gray-600">
          <button v-on:click="submitGroup()" data-modal-toggle="default-modal" type="button" class="focus:ring-4 focus:ring-gray-300 rounded-lg border  text-sm font-medium px-5 py-2.5  focus:z-10 text-white hover:bg-gray-600">Create</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    closeModal() {
      this.$store.dispatch('submitGroupModal/closeModal')
    },
    submitGroup() {
      const _self = this;
      this.group.location_id = this.location.id;
      this.$store.dispatch('groups/createPublicGroup', {group: this.group, auth: this.token}).then(function(){
        _self.closeModal();
      })
    }
  },
